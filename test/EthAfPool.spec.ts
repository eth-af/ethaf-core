...
import { TestEthAfSwapPay } from '../typechain/TestEthAfSwapPay'
...
import { TestEthAfReentrantCallee } from '../typechain/TestEthAfReentrantCallee'
...
describe('EthAfPool', () => {
  ...
  let pool: MockTimeEthAfPool

  let swapTarget: TestEthAfCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeEthAfPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestEthAfReentrantCallee')
      ).deploy()) as TestEthAfReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestEthAfSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestEthAfSwapPay')
      underpay = (await underpayFactory.deploy()) as TestEthAfSwapPay
