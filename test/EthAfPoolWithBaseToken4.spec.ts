...
describe('EthAfPoolWithBaseToken4', () => {
  ...
  describe("distributor", function () {
    ...
    it('can loop over multiple pools', async function () {
      ...
      await expect(p1).to.emit(pool1, "BaseTokensCollected").withArgs(amount1_0, 0)
      await expect(p1).to.emit(pool2, "BaseTokensCollected").withArgs(amount1_1, 0)
      ...
      await expect(p2).to.not.emit(pool1, "BaseTokensCollected")
      await expect(p2).to.not.emit(pool2, "BaseTokensCollected")
      await expect(p2).to.not.emit(pool3, "BaseTokensCollected")
      await expect(p2).to.not.emit(pool4, "BaseTokensCollected")
      await expect(p2).to.not.emit(pool5, "BaseTokensCollected")
      await expect(p2).to.not.emit(pool6, "BaseTokensCollected")
