...
import { EthAfPoolSwapTest } from '../typechain/EthAfPoolSwapTest'
...
describe('EthAfPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('EthAfPoolSwapTest')
            const tester = (await testerFactory.deploy()) as EthAfPoolSwapTest
