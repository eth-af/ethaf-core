...
contract TestEthAfRouter is IEthAfSwapCallback {
    ...
    function ethafSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IEthAfPool(msg.sender).token0() : IEthAfPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IEthAfPool(pools[0]).token1();
            IEthAfPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IEthAfPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IEthAfPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
